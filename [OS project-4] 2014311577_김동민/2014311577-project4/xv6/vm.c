...
int handle_pgfault(struct proc *p, uint faultaddr)
{
  ...
  if((pte = walkpgdir(p->pgdir, (char*)pg_loc, 0)) == 0)
    ...
    // PTE_P were to be 1, no page fault would have occured.

    if(*pte & PTE_P) return -1;
    if(!(*pte))
      panic("Pagefault Handler: PTE_P is 0, and No data for swapspace.");
