...
uint mmap(uint addr, int length, int prot, int flags, int fd, int offset)
{
  ...
  */
  ...
  // exceptional case: addr is null, but flag has MAP_FIXED
...
int handle_pgfault(struct proc * p, uint faultaddr, uint errorcode)
{
  ...
  // error code other than read/write flags
  if(errorcode != 1 && (errorcode & 2) != 0)
    return -1;
  ...
  if(((errorcode & 2) == 1) && !(mmentry->prot & PROT_WRITE)) return -1; 
...
int munmap(uint addr)
{
  ...
  */
  ...
  // find according mapping region in mmlist
  for(i = 0; i < mmlen; i++){
    if((mmlist[i].p)->pid == curproc->pid && pagebelongs(addr, mmlist[i].addr, mmlist[i].length)){
      in_mmlist = 1;
      mmentry = &mmlist[i];
      break;
    }
